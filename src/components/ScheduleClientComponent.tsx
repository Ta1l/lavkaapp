...
type Props = {
  initialWeekDays: Day[] | any[]; // может быть сериализовано с сервера
...
export default function ScheduleClientComponent({
  ...
  // Нормализуем начальные дни: если date пришёл строкой — превратим в Date
  const normalizedInitialWeekDays: Day[] = (initialWeekDays ?? []).map((d: any) => ({
    ...d,
  ...
  const [weekDays, setWeekDays] = useState<Day[]>(normalizedInitialWeekDays);
  ...
  useEffect(() => {
    setWeekDays(normalizedInitialWeekDays);
    setLoading(false);
  }, [initialWeekDays?.length]); // обновляем при изменении приходящих данных

  const navigate = (newOffset: number) => {
    if (loading) return;
    setLoading(true);
    const params = new URLSearchParams(searchParams?.toString() ?? "");
    router.push(`/schedule/${newOffset}${params.toString() ? `?${params.toString()}` : ""}`);
  };
  ...
  const handleModalDone = async (startTime: string, endTime: string) => {
    ...
    try {
      ...
      if (!res.ok) throw new Error(json.error || "Не удалось создать слот");
      refreshData();
    } catch (err) {
  ...
  const handleTakeSlot = async (day: Day, slot: TimeSlot) => {
    ...
    try {
      ...
      const json = await res.json();
      if (!res.ok) throw new Error(json.error || "Не удалось занять слот");
      refreshData();
    } catch (err) {
      alert(getErrorMessage(err));
      setLoading(false);
  ...
  const handleReleaseSlot = async (day: Day, slotId: number) => {
    ...
    try {
      ...
      const json = await res.json();
      if (!res.ok) throw new Error(json.error || "Не удалось освободить слот");
      refreshData();
    } catch (err) {
      alert(getErrorMessage(err));
      setLoading(false);
  ...
  const handleDeleteDaySlots = async (day: Day) => {
    ...
    try {
      ...
      const json = await res.json();
      if (!res.ok) throw new Error(json.error || "Не удалось удалить слоты");
      refreshData();
    } catch (err) {
      alert(getErrorMessage(err));
      setLoading(false);
  ...
  return (
    <>
      ...
      {isModalOpen && selectedDay && (
        <AddSlotModal onClose={() => setIsModalOpen(false)} onDone={handleModalDone} />
      )}
    </>
  );
}
